# BoneRAG Grounded VQA Benchmark v5

## Purpose

Measure whether foundation-model image retrieval and evidence-grounded answering
help closed visual question answering for bone pathology. The current runnable
protocol is a **FracAtlas-derived grounded VQA pilot**: questions are generated
from FracAtlas annotations and scored against the held-out image label.

This is not a native clinician-authored VQA benchmark and not a direct
reproduction of RULE, MMed-RAG, FactMM-RAG, MR-RAG, MKGF, Path-RAG, or VisRAG.

## Dataset and split

- Active dataset: FracAtlas, mounted in Colab under
  `BONERAG_DATASET_IMAGES_ROOT`.
- Active task: label-derived closed fracture VQA. The yes/no question is
  generated by this project from the FracAtlas folder label and optional
  annotation metadata.
...
- Fingerprint: every run reports a SHA-256 fingerprint of the selected paths and
  labels.
- Grounding status: FracAtlas boxes/masks are recorded when available, but the
  current systems do not output query-image boxes/masks. Therefore localization
  metrics such as IoU/Dice/mAP are **not scored yet**.

## Dataset roadmap

The assignment datasets are mainly image-annotation datasets, so VQA is built
from their labels/boxes/masks:
...
- `grounding_reference_cases`: how many test cases have reference region
  annotations available.
- `grounding_scored`: currently `false`; turns true only after systems emit
  query-image regions and those regions are scored against boxes/masks.
...
- benchmark scope and grounded-VQA manifest showing which datasets/tasks are
...
3. RadBench or ImageCLEF VQA-Med musculoskeletal subset as external native-VQA
   validation.
4. Query-image lesion-region output from BoneRAG so IoU/Dice/mAP can be scored.
